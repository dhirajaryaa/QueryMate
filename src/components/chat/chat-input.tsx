...
export default function ChatInput({ sendMessage }: { sendMessage: (message: string) => void }) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="flex items-center justify-between gap-2 rounded-4xl border shadow p-2 w-full h-auto">
                <Input
                    value={isPrompt}
                    onChange={(e) => setIsPrompt(e.target.value)}
                    className="border-0 outline-0 focus-visible:ring-0 bg-transparent shadow-none dark:bg-transparent"
                />
